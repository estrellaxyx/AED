...
public class Transportistas {
	...
	public PositionList<Point> bestPathFromTo(Point fromPoint, Point toPoint) {
		...
		for (Vertex<Point> v : graph.vertices()) {
			if (points2Vertices.get(fromPoint) == v)
				pq.enqueue(0, v);
			else
	...
	private void dijkstra(PriorityQueue<Integer, Vertex<Point>> pq,
		...
		if (pq.isEmpty())
			return;
		Vertex<Point> vMin = pq.dequeue().getValue();
		Integer costeMin = pq.dequeue().getKey();
	...
	public PositionList<Point> camino(Vertex<Point> from, Vertex<Point> to, Map<Vertex<Point>, Edge<Integer>> froms) {
		PositionList<Point> res = new NodePositionList<Point>();
		if (from == to)
			return res;
		for (Vertex<Point> v : froms.keys()) {
